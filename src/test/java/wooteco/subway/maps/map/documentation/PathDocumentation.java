...
public class PathDocumentation extends Documentation {
    ...
    void findPath() {
        List<StationResponse> stationResponses = Arrays.asList(
                new StationResponse(1L, "가", LocalDateTime.now(), LocalDateTime.now()),
                new StationResponse(3L, "다", LocalDateTime.now(), LocalDateTime.now()),
                new StationResponse(2L, "나", LocalDateTime.now(), LocalDateTime.now()),
                new StationResponse(4L, "라", LocalDateTime.now(), LocalDateTime.now())
        );
        PathResponse pathResponse = new PathResponse(stationResponses, 20, 10, 30);
        when(mapService.findPath(any(), any(), any(), any()))
